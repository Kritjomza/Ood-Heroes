...
To replace a mock, create the documented file at its `replacementPath`, update the renderer behind that Asset ID, and keep gameplay and screen code unchanged. Hero production exports should retain transparent backgrounds, consistent optical scale, and all required directional animation frames. Missing IDs resolve to a visible `?` fallback and never crash a screen.
